...
def _deployed_model_id(session_factory: sessionmaker[Session]) -> str:
    with session_factory() as session:
        model = ModelRepository(session).get_by_name("test-model")
        assert model is not None
        return str(model.id)


def _create(client: TestClient, model_id: str, *, name: str = "exp-a") -> dict[str, object]:
    response = client.post(
        "/experiments",
        json={
            "name": name,
            "model_id": model_id,
            "generation_config": {"num_beams": 1, "max_new_tokens": 32, "max_input_tokens": 128},
        },
    )
...
def test_create_returns_201_and_echoes_config(client: TestClient, session_factory: sessionmaker[Session]) -> None:
    model_id = _deployed_model_id(session_factory)

    body = _create(client, model_id)

    assert body["name"] == "exp-a"
    assert body["model_id"] == model_id
    assert body["generation_config"] == {"num_beams": 1, "max_new_tokens": 32, "max_input_tokens": 128}


def test_create_rejects_unknown_generation_knob(client: TestClient, session_factory: sessionmaker[Session]) -> None:
    model_id = _deployed_model_id(session_factory)

    response = client.post(
        "/experiments",
        json={"name": "bad", "model_id": model_id, "generation_config": {"temperature": 0.2}},
    )
...
def test_create_with_unknown_model_returns_404(client: TestClient) -> None:
    response = client.post("/experiments", json={"name": "ghost", "model_id": _UNKNOWN_ID})
...
def test_run_returns_the_inference(client: TestClient, session_factory: sessionmaker[Session]) -> None:
    experiment = _create(client, _deployed_model_id(session_factory), name="run-exp")
...
def test_results_are_empty_before_evaluation(client: TestClient, session_factory: sessionmaker[Session]) -> None:
    experiment = _create(client, _deployed_model_id(session_factory), name="results-exp")
