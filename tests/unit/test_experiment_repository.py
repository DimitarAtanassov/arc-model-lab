...
async def test_add_translates_duplicate_name_to_conflict() -> None:
    session = MagicMock(spec=AsyncSession)
    session.flush.side_effect = _integrity_error("uq_experiments_name")

    with pytest.raises(ExperimentNameConflictError):
        await ExperimentRepository(session).add(_experiment())


async def test_add_reraises_non_name_integrity_error() -> None:
...
async def test_add_reraises_when_constraint_name_is_absent() -> None:
...
async def test_get_raises_on_corrupt_stored_config() -> None:
    ...
    with pytest.raises(CorruptStoredDataError):
        await ExperimentRepository(session).get(uuid4())
