...
async def list_models(session: SessionDep, service: ServiceDep) -> list[ModelResponse]:
    return [ModelResponse.from_domain(model) for model in await service.list_models(session)]


@router.get("/{name}", response_model=ModelResponse)
async def get_model(name: str, session: SessionDep, service: ServiceDep) -> ModelResponse:
    """Return one catalog model by its unique name, or 404 if absent."""
    return ModelResponse.from_domain(await service.get(session, name))
